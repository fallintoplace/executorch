...
def test_int16_table_small_output_range_is_not_degenerate():
    ...
    # generate_16_bit_table_values does not use ``self``; construct an
    # uninitialized instance to call it (see test_insert_table_ops_pass.py).
    pass_instance = object.__new__(InsertTableOpsPass)
    lut, rescale_lshift = pass_instance.generate_16_bit_table_values(
...
def test_int16_table_large_output_range_applies_rshift():
    """The ``max(rshift, 0)`` clamp must be a no-op when rshift is already positive.

    Complements test_int16_table_small_output_range_is_not_degenerate (which
    covers rshift < 0). Here an int32-output TABLE has a max table value above
    2**16, so the pre-clamp rshift is > 0 and the values genuinely must be
    shifted right to fit int16 -- the path the clamp must NOT regress. A buggy
    clamp such as ``min(rshift, 0)`` or ``abs(rshift)`` would change the shift
    and this test would catch it.
    """
    # out scale 1/70000 -> max table value 70000 (the code's own worked example),
    # so rshift = ceil(log2(70000)) + 1 - 16 = 17 + 1 - 16 = 2 (positive).
    in_quantargs = QuantArgs(
        scale=0.0006833122461102903, zp=0, qmin=-32767, qmax=32767, dtype=torch.int16
    )
    out_quantargs = QuantArgs(
    ...
    assert (
        torch.unique(lut).numel() > 1
    ), "int16 table collapsed to a constant for a large (int32) output range"
    assert bool(
        (lut[1:] >= lut[:-1]).all()
    ), "sigmoid table must be monotonically non-decreasing"
    # rshift = 2 is positive, so max(rshift, 0) leaves it unchanged: the table is
    # shifted right by 2 (70000 >> 2 == 17500) and rescale_lshift == 2 - 7.
    assert (
        rescale_lshift == -5
    ), f"expected rescale_lshift == -5 (positive rshift=2 applied), got {rescale_lshift}"
    assert (
        0 < int(lut.max()) < 2**15
    ), f"expected table shifted into int16 range (0, 2**15); got max={int(lut.max())}"
